...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _send2FACode(BuildContext context) async {
    if (_formKey.currentState!.validate()) {
      setState(() => _isLoading = true);
      final auth = Provider.of<MershadAuthProvider>(context, listen: false);
      try {
        await auth.send2FACode(_emailController.text);
        setState(() {
          _isLoading = false;
          _errorMessage = 'Please check your email for the 2FA verification link.';
        });
      } catch (e) {
        setState(() {
          _errorMessage = 'Failed to send 2FA code: ${e.toString()}';
          _isLoading = false;
        });
      }
    }
  ...
  void _verify2FA(BuildContext context) async {
    setState(() => _isLoading = true);
    final auth = Provider.of<MershadAuthProvider>(context, listen: false);
    try {
      bool success = await auth.verify2FA();
      if (success) {
        Navigator.pushReplacementNamed(context, AppRoutes.home);
      } else {
        setState(() {
          _errorMessage = 'Email not verified yet. Please click the link in your email.';
          _isLoading = false;
        });
      }
    } catch (e) {
                              ...
                              children: [
                                if (!_show2FAStep) ...[
                                  ...
                                  _buildTextField(
                                    ...
                                    icon: Icons.email,
                                    validator: Validator.validateEmail,
                                    enabled: true, // Disable editing in 2FA step
                                  ),
                                ],
                                ...
                                if (!_show2FAStep) ...[
                                  ...
                                  _buildLoginButton(
                                    text: _isLoading ? 'LOADING...' : 'SEND 2FA EMAIL',
                                    onPressed: _isLoading ? null : () => _send2FACode(context),
                                    backgroundColor: Colors.white,
                                    textColor: primaryColor,
                                  ),
                                  SizedBox(height: 16),
                                  _buildLoginButton(
                                    text: _isLoading ? 'LOADING...' : 'VERIFY 2FA',
                                    onPressed: _isLoading ? null : () => _verify2FA(context),
                                    backgroundColor: Colors.white.withOpacity(0.8),
                                    textColor: primaryColor,
                                  ),
                                ],
